...
test('manufacturer submits a bicycle and admin publishes it to the catalog', async ({ page }) => {
  ...
  await page.getByRole('row').filter({ hasText: bicycleTitle }).getByRole('link', { name: 'Open' }).click()
  ...
  await expect(page.getByRole('row').filter({ hasText: renterEmail }).first().getByText('succeeded')).toBeVisible()
